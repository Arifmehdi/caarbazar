    ...
    <div class="container">
        <div class="row mt-5">
            ...
            @forelse($infos as $sear)
                @php
                    $image_obj = $sear->local_img_url;
                    $images = explode(',', $sear->local_img_url);
                                ...
                                <div class="item-card9-imgs">
                                    ...
                                    @if (!empty($image_obj) && $image_obj != '[]')
                                        <a title="{{ $sear->title }}"
                                            ...
                                            class="text-dark"></a>
                                            <img class="auto-ajax-photo-fav" src="{{ asset('frontend/') }}/{{ $image }}"
                                    ...
                                    @elseif ($image_obj == '[]')
                                        <a href="{{ route('auto.details', ['vin' => $vin_string_replace, 'param' => $route_string]) }}" class="text-dark"></a>
                                            <img class="auto-ajax-photo-fav" width="100%"
                                                src="{{ asset('frontend/NotFound.png') }}"
                                                alt="Used cars for sale {{ $sear->title }}, price is {{ $sear->price }}, vin {{ $sear->vin }} in {{ $sear->dealer->city }}, {{ strtoupper($sear->dealer->state) }}, dealer name is {{ $sear->dealer->name }} Best Dream car image ">

                                    @else
                                        <img class="auto-ajax-photo-fav" width="100%"
                                            src="{{ asset('frontend/NotFound.png') }}"
                                            alt="Used cars for sale {{ $sear->title }}, price is {{ $sear->price }}, vin {{ $sear->vin }} in {{ $sear->dealer->city }}, {{ strtoupper($sear->dealer->state) }}, dealer name is {{ $inventory->dealer->name }} Best Dream car image ">
                                    @endif
                                </div>
