...
class CompareController extends Controller
{
    ...
    public function index(){
        $ip = request()->ip();
        $items = Compare::with('inventory')->where('ip',$ip)->get();
...
public function collect(Request $request) {
    $coms = Compare::with('inventory')->where('ip', $request->ip())->get();
    ...
    foreach ($coms as $com) {
        $image_obj = $com->inventory->local_img_url;
        ...
        $coms_array[] = [
            'id' => $com->id,
            'title' => truncateTitle($com->inventory->title, 3), // Truncate title to 15 words
            'image_path' => $image_path,
            'price' => $com->inventory->price_formate // Include price
        ];
    }
