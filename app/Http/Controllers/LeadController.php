...
class LeadController extends Controller
{
    ...
    public function lead(Request $request)
    {
        ...
        $image_str = str_replace(['[', ' ', "'"], '', $inventory->local_img_url);
    ...
    {
        $lead_details = Lead::with('inventory')->where('user_id', $userInfo['id'])->orderBy('id', 'desc')->first();

        $year = $lead_details->inventory->year;
        $make = $lead_details->inventory->make;
        $model = $lead_details->inventory->model;
        $image = $lead_details->inventory->local_img_url;
    ...
    public function leadShow(Request $request, $id = null)
    {
        ...
        $leads = Lead::with('customer')->orderBy('id', 'desc')->get();
        // $salesmans = Sale::all();
        ...
        if ($request->ajax()) {
            return datatables::of($info)->addIndexColumn()
                ->addColumn('DT_RowIndex', function ($user) {
                    ...
                    return $row->inventory->title ?? 'Null';
                })
                ->addColumn('make', function ($row) {
                    return $row->inventory->make ?? 'Null';
                })
    ...
    {
        $lead = Lead::with('inventory', 'customer', 'dealer')->find($id);
        $lead->status = '1';
        $lead->save();
    ...
    {
        $lead_details = Lead::with('inventory','customer')->where('id', $lead_id)->first();
