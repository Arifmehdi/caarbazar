...
class ProductServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton('priceRange', function ($app) {
            $newData = DB::select('SELECT MIN(price) AS min_price, MAX(price) AS max_price, MIN(miles) as min_miles, MAX(miles) as max_miles FROM inventories WHERE type = "new"');
            $usedData = DB::select('SELECT MIN(price) AS min_price, MAX(price) AS max_price, MIN(miles) as min_miles, MAX(miles) as max_miles FROM inventories WHERE type != "new"');
            return [
                'new' => [
